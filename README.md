<p align="letf">

  <img src="https://komarev.com/ghpvc/?username=anpilogov-com&label=Profile%20views&color=2bbc8a&style=flat" alt="anpilogov-com" />
  <img src="https://img.shields.io/badge/Linux-OS-_.svg?style=flat&logoColor=ffffff&color=2bbc8a&logo=data:image/webp;base64,">
  <img src="https://img.shields.io/badge/Windows-OS-_.svg?style=flat&logoColor=black&color=2bbc8a&logo=data:image/webp;base64,">

</p>


<h1 align="center">Привет, я Артём <img src="https://raw.githubusercontent.com/MartinHeinz/MartinHeinz/master/wave.gif" width="30px"></h1>

<p align="center">
  🧑‍💻 Frontend-разработчик с 5-летним опытом<br/>
  🌍 Работаю с Vue 3, Nuxt 3, Nest, TypeScript, Tailwind<br/>
  ⚙️ Настраиваю CI/CD, пакую контейнеры<br/>
  🔍 Участвую в архитектуре проектов и ревью кода
</p>

---

### 🚀 Технологии и стек

- **Языки:** TypeScript, JavaScript, PHP, Python
- **Фреймворки:** Vue 3, Nuxt 3, Vite, Pinia, Nest, Reat и д.р.
- **Стили:** Tailwind CSS, SCSS, BEM
- **Тестирование:** Selenium + Python, Vitest, Cypress
- **DevOps:** Docker, GitLab CI/CD, Kubernetes (в процессе)
- **CMS/CRM:** WordPress

---

### 💼 Текущее место работы

> 🏢 **TwinPx** — Fullstack-разработчик (с октября 2025)  
> Разрабатываю и поддерживаю web-интерфейсы, участвую в проектировании архитектуры web-приложений.

> 🏢 **Свой преокт** — Fullstack-разработчик (с октября 2024)  
> Полный цикл разработки продукта по обеспечению сетевой безопасности.

---

### 🌱 Сейчас изучаю

- Kubernetes и CI/CD пайплайны
- Архитектурные паттерны (DDD, FSD)
- Расширенная интеграция с Bitrix24 API
- Go и не только

---

### 📬 Связь со мной

- Telegram: [@anpilogov_artyom](https://t.me/anpilogov_artyom)
- Email: a.o.anpilogov@yandex.ru
